...
export default function Home() {
  ...
  return (
    <main className="flex flex-col min-h-screen relative overflow-x-hidden cursor-default">
      ...
      {/* logo screen */}
      <div className="relative w-full h-screen flex flex-col justify-center items-center bg-white dark:bg-black z-10 pointer-events-auto">
        <BackgroundGradientAnimation />
        <Link href="/lab">
          <div className="flex flex-col justify-center items-center p-4 md:p-10 lg:flex-row lg:gap-10 rounded-xl bg-white dark:bg-black ring-1 ring-inset ring-neutral-200 dark:ring-neutral-800 pointer-events-auto cursor-pointer transition-all dark:hover:ring-purple-700 dark:hover:ring-2 hover:scale-105 absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2">
            <HeroHighlight className="flex flex-col lg:flex-row items-center gap-5 md:gap-10">
              <motion.div initial={{ filter: "blur(5px)", opacity: 0 }} whileInView={{ filter: "blur(0)", opacity: 1 }} viewport={{ once: true }} transition={{ duration: .5, delay: 0, ease: "linear" }}>
                <p className="text-center md:text-left text-2xl md:text-[3rem] md:leading-[1.1] font-semibold tracking-tight text-black dark:text-white">
                  <Highlight>Meld</Highlight> {t('logoCard.title')}
                </p>
              </motion.div>
              <Image
                priority
                ...
                width={375}
                height={375}
                className="w-full md:w-1/2 max-w-full max-h-full object-contain pointer-events-none select-none"
              />
            </HeroHighlight>
          </div>
        </Link>
      ...
      {/* nextjs & react */}
      <div className="relative flex flex-col md:flex-row w-full h-[75vh]">
        <div className="h-full w-full relative flex flex-col items-center justify-center dark:bg-black bg-white dark:bg-grid-white/[0.2] bg-grid-black/[0.2] gap-8">
          ...
          <motion.p className="relative max-w-96 text-lg font-semibold leading-tight tracking-tight text-neutral-800 dark:text-neutral-200 px-4 text-center"
            initial={{ opacity: 0, y: 25 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: .5, delay: 0, ease: "easeInOut" }}>
            {t.rich('nextjsText', {
      ...
      {/* tailwind */}
      <div className="h-auto w-screen flex flex-col items-center bg-white dark:bg-black gap-8">
        <motion.div
        ...
        >
          <p className="z-20 mt-8 text-5xl md:text-7xl font-bold text-center leading-tight tracking-tight from-[#38bdf7] to-[#0531f1] bg-gradient-to-b bg-clip-text text-transparent">
            Tailwind
          </p>
        ...
        <motion.p
          className="max-w-md mx-8 mb-8 text-lg font-semibold leading-tight tracking-tight text-center from-sky-600 to-blue-700 bg-gradient-to-b bg-clip-text text-transparent"
